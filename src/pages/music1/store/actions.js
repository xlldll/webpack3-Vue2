...
export default{
	 ...
	 */
		...
		API.searchSongs('远走高飞').then((data) => {
	 ...
	 * @param dispatch
	 */
			[type.UPD.searchLrc]({state, commit, dispatch}){
		let id = state.currentTrackInfo.id;

		API.searchLrc(id).then(({data}) => {
			if (data.uncollected || data.nolyric) {
				commit(type.CHG.nolyric, true);
			}
			if (data.lrc) {
				commit(type.CHG.lrc, data.lrc.lyric);
			}
		});
	},
}
